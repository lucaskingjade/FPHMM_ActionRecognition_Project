...
fprintf('size of allDataCell is [%d,%d]\n',size(allDataCell,1),size(allDataCell,2));
fprintf('size of allData is [%d,%d]\n',size(allData,1),size(allData,2));

%remove rows that contains nan from all data sequences
disp('begin removing all the nan rows from data')
[allData,rowIndex ]= rmNANrow(allData);
for indAct =1:numActivity
    for indEm = 1:numEmotion
        numFl = size(allDataCell{indAct,indEm},1);
        for indFl = 1:numFl
            allDataCell{indAct,indEm}{indFl,1} = rmNANrow(allDataCell{indAct,indEm}{indFl,1},rowIndex);
        end
    end
end

fprintf('size of allData is [%d,%d]\n',size(allData,1),size(allData,2));
disp('begin to do PCA\n');
if isPCA
    dim = 16;
...
[CrossValDataSet,testDataSet] =divideDataSet(allDataCell,3);
allDataCell = cell(numActivity,numEmotion);

allDataCell = CrossValDataSet;
...
%save K_TrainingSet,K_TestingSet,CrossValDataSet,testDataSet
