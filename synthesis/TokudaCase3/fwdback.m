...
%  if(t<Q) llh = llh + sum(alpha(1:t,t));
%  elseif(T-t<Q) llh = llh + sum(alpha(Q-(T-t):Q,t));
%  else llh = llh + sum(alpha(:,t));
%  end
...
%force etat final
end_state_distrib=zeros(1,Q);
end_state_distrib(end)=1;
alpha(:,T) = end_state_distrib(:) .* obslik(:,T);
if scaled
 %[alpha(:,t), scale(t)] = normaliseC(alpha(:,t));
 [alpha(:,T), scale(T)] = normalise(alpha(:,T));
end
...
%    loglik
   %loglik = sum(log(scale(1:end-1)))+log(alpha(Q,T));
 end
else
