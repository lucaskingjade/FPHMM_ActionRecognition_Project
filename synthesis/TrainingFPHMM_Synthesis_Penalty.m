...
%         break;!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    Q = numStates;
	%% compute two terms for every sequences in training dataset
    term1 = cell(numActivity,numEmotion);
    term2 = cell(numActivity,numEmotion);
    ...
    for indAct =1:numActivity
        ...
        for indEm = 1:numEmotion
            ...
            tmpDataCell = TrainDataSet{indAct,indEm};
            curnumFile = size(tmpDataCell,1);
            term1{indAct,indEm} = cell(curnumFile,1);
            term2{indAct,indEm} = cell(curnumFile,1);

            index_front = index_back+ 1;
            index_back = index_back + curnumFile;
            gammaSetOfOneEmotion = gammaCell{indAct,1}(index_front:index_back,1);
            
            for i = 1:curnumFile
                cellSet2CHMM = cell(2,Q,1);
                for q = 1:numStates
                    cellSet2CHMM{1,q,1} = tmpDataCell{i,1};
                    cellSet2CHMM{2,q,1} = gammaSetOfOneEmotion{i,1}(q,:);
                end
                thetasCell  = getElements4CalculateThetaofCommonMeanParametricHMM(cellSet2CHMM,wSet,commonMean,invOfSigma);
                
                term1{indAct,indEm}{i,1} = thetasCell{1,1};
                term2{indAct,indEm}{i,1} = thetasCell{2,1};
            end
        end
    end
    
    %% compute theta for all emotions
    for indEm =1:numEmotion
        sum_term1_1 = 0;
        sum_term2_1 = 0;
        sum_term1_2 = 0;
        ...
        %compute 
        parfor indAct =1:numActivity
            if isempty(term1{indAct,indEm})
                continue;
            end
            curnumFl = size(term1{indAct,indEm},1);
            M=M+curnumFl;
            for i = 1:curnumFl
                sum_term1_1 = sum_term1_1 + term1{indAct,indEm}{i,1};
                sum_term2_1 = sum_term2_1 + term2{indAct,indEm}{i,1};
            end
        end
        
        parfor indEm_1 = 1:numEmotion
            if indEm_1 ==indEm
                continue;
            end
            for indAct_1 = 1:numActivity
                if isempty(term1{indAct_1,indEm_1})
                    continue;
                end
                tmpNumFile = size(term1{indAct_1,indEm_1},1);
                N = tmpNumFile+N;
                for ii =1:tmpNumFile
                    sum_term1_2 = sum_term1_2 + term1{indAct_1,indEm_1}{ii,1};
                    sum_term2_2 = sum_term2_2 + term2{indAct_1,indEm_1}{ii,1};
                end
            end
        end
        % compute theta
        
        contextualVector{indEm,1} = (sum_term1_1-Lambda*M/N*sum_term1_2)^-1 *...
                  (sum_term2_1-Lambda*M/N*sum_term2_2);
